...
export default function FloatingChat2() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <div
        className={`${
          isOpen ? 'hidden' : 'block'
        } bg-purple-600 hover:bg-purple-700 text-white p-4 rounded-full cursor-pointer shadow-lg transition duration-300 ease-in-out`}
        onClick={toggleChat}
      >
      ...
      >
              ...
              <div
                className={`inline-block p-4 rounded-md max-w-[75%] transition duration-300 ease-in-out transform hover:scale-105 ${
                  message.role === 'user'
                    ...
                    : 'bg-purple-200 text-gray-800 shadow-md hover:bg-purple-300'
                }`}
              >
                {message.content}
              </div>
        ...
        <div className='flex p-4'>
          ...
          />
          {isLoading ? (
            <div className='bg-purple-500 text-white p-3 rounded-sm shadow-l-2xl animate-pulse'>
              Loading...
            </div>
          ) : (
            <button
              ...
              className={`px-6 py-3 font-sans ${isLoading ? 'opacity-50 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white border-transparent rounded-r-lg'}`}
            >
